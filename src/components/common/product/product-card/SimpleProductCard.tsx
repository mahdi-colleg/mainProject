...
export function SimpleProductCard({data}: Props) {
    return (
        <div
            className="group flex flex-col justify-between border-[1px] border-gray-200 hover:border-green-150 rounded-[10px] hover:shadow-[20px_20px_40px_0_rgba(24,24,24,0.07)] relative p-3 md:p-4 xl:px-5 xl:pb-5 lg:pt-[65px] min-h-[420px]">
            {/* --- Badge --- */}
            ...
            {/* --- Product Image --- */}
            <ImageView src={data.attributes.thumbnail?.data?.attributes.url} alt="popular" width={210} height={168}
            ...
            <div className="flex flex-col gap-2 flex-1">
                {data.attributes.categories?.data[0] ? (
                    <div
                        className="text-gray-500 text-xsmall">{data.attributes.categories.data[0].attributes.title}</div>
                ) : (
                    <div className="text-gray-500 text-xsmall">none</div>
                )}

                <Link href="#">
                    <h3 className="text-heading-sm text-blue-300 max-h-[50px] overflow-hidden">{data.attributes.title}</h3>
                </Link>
