    ...
    return (
        <>
            <div className={"relative"}>
                ...
                <div onClick={browsCategoryBtnHandler} className="inline-flex cursor-pointer bg-green-200 gap-2.5 text-white px-4 py-3 rounded-[5px] items-center">
                ...
                <div onClick={browsItemHandler} className={`${showCategoryMenu ? "flex" : "hidden"} lg:absolute z-20 bg-white left-0 top-16 lg:w-[500px] rounded-[5px] lg:border-[1px] border-green-300 lg:p-[30px] hover:cursor-default`}>
                    <div id="all_cat_inner_box" className="flex flex-wrap justify-between gap-y-[15px]">

                        {
                            ...
                            browsCategory.data.map((item:EntityType<MenuItemType>, index:number) => {
                                    return (
                                        <IconBox key={index} link={item.attributes.link} icon={item.attributes.icon_name} size={30} title={item.attributes.title} titleClassName={"text-heading-sm text-blue-300"} path={item.attributes.icon_path} linkClassName={"gap-3.5 rounded-[5px] lg:border-[1px] lg:border-gray-300 py-2.5 basis-full lg:basis-[calc(50%-8px)] justify-start pl-4 lg:hover:border-green-300"}/>
                                    )
                                }
                            )
                        }

                        <IconBox link={"#"} icon={"icon-add"} size={24} title={"More Categories"} titleClassName={"text-heading-sm text-blue-300"} linkClassName={"gap-4 lg:justify-center w-full mt-[17px]"}/>
                    </div>
                </div>
            </div>
